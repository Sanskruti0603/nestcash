...
import { MdOutlineEmail, MdDateRange, MdCancel } from "react-icons/md";
import { BsGraphUp, BsFileText } from "react-icons/bs";
import { motion } from "framer-motion";

const RejectedLoan = ({ setShowRejectedLoan }) => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setLoans(response.data.data.rejectedLoan || []);
      } catch (error) {
        console.log(error);
      }
    };
    ...
    >
      ...
      >
        <button
          ...
          onClick={() => setShowRejectedLoan(false)}
        >
          &times;
        </button>
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {loans.map((loan) => (
              ...
              >
                ...
                <div className="text-red-700 text-sm font-semibold flex items-center mt-2">
